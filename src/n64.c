...
static void ExtractFrustum(float frustum[6][4], float proj[16], float modl[16]) {
	float clip[16];
	float t;
...
static bool PointInFrustum(float frustum[6][4], Vec3f* pos) {
	int p;
	
	for ( p = 0; p < 6; p++ )
		if ( frustum[p][0] * pos->x + frustum[p][1] * pos->y + frustum[p][2] * pos->z + frustum[p][3] <= -500.0f )
			return false;
...
static bool gbiFunc_culldl(void* cmd) {
	...
	// MtxF m;
	// Matrix_MtxFMtxFMult(proj, view, &m);
	ExtractFrustum(frustum, (float*)proj, (float*)view);
	
	for (i = vfirst; i < vlast; ++i, ++v) {
		// multiply by view/projection matrices
		Vec3f* pos = (Vec3f*)&v->pos;
		
		if (PointInFrustum(frustum, pos))
			return false;
	}
